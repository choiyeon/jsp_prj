...
// 			history.back();
			location.href="http://192.168.10.218/jsp_prj/board/board_list.jsp?currentPage=${ param.currentPage }";
		});//click
	...
	function searchReply(){
		...
		$.ajax({
			...
			success : function(jsonObj){
				var output="댓글을 읽어올 수 없습니다. 잠시후 다시 시도해주세요";
				if(jsonObj.flag){
					
					output="";
					if(jsonObj.dataSize == 0){
						//댓글이 없을 경우
						output = "작성된 댓글이 없습니다. 당신의 의견을 작성해보세요.";
					}//end if
					...
					$.each(jsonObj.data, function(i, jsonTemp){
						...
						output += "<strong>작성자</strong> : " + jsonTemp.writer 
								...
								+ "<input type='button' value='삭제' class='btn btn-warning btn-sm' onclick='removeReply("+jsonTemp+")'/>"
								+ "</div>";
					});//each
				}//end if
...
BoardDAO bDAO = BoardDAO.getInstance();
	try{
		...
		//상세보기
		BoardVO bVO = bDAO.selectDetailBoard(Integer.parseInt(seq));
		//조회수 올리기
...
%>
	...
	%>
	<script type="text/javascript">
	location.href="http://192.168.10.218/jsp_prj/error/err_500.html";
	</script>
	<%
	}//end catch
%>
	...
	<tr>
		...
		<c:if test="${ bVO.id eq sessionScope.loginData.id }"><!-- 로그인된 상태이고, 글을 쓴 사람한테만 수정&삭제버튼 보이기 -->
