...
<td>
<%-- <%for(int j=0; j < 11; j++) { %> --%>
<%-- 	<input type="radio" name="score_<%= i %>" <% if(j==5){ %> checked="checked" <% } %>><%= j %> --%>
<%-- 	<input type="radio" name="score_<%= i %>" <%= j==5?" checked='checked' " : ""  %> <% } %>><%= j %> --%>
<%= createRadio(10, "score_", i, 5) %>
</td>
</tr>
<% }//end for %>
</table>
