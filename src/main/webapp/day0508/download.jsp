...
<%
	...
	//2.응답헤더 변경 ( 사용자가 선택한 파일의 이름으로 다운로드파일을 설정)
	//파일명이 한글인 경우 한글파일로 다운로드되지 않는다. => URL에서 사용할 수 있도록 파일명을 encoding
	response.setHeader("Content-Disposition", "attachment;fileName="+
	URLEncoder.encode(downloadName,"UTF-8"));
	//3.다운로드할 파일에 정보를 얻기
	File file
		=new File("D:/dev/workspace/jsp_prj/src/main/webapp/upload/"+downloadName);
	
	if(file.exists()){
		FileInputStream fis=null; //파일에서 읽기위한 스트림
		OutputStream os=null; //파일을 웹 브라우저로 출력하기위한 스트림
		try{
		//4.IOStream연결
