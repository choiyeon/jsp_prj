			...
			if($("#car_country").val() != "N/A"){
				...
				$.ajax({
					...
					success: function(jsonObj){
						...
						if(!jsonObj.result){
							alert("검색이 원활하지 않습니다. 잠시후 다시 시도해주세요.");
							return;
						}//end if
						
						//select의 options들은 동적으로 생성
						//1. select 얻기
						...
						makerSel.length=1;
						//3. option들을 생성
						// select객체.options[i] = new Option("보여질 값", "속성 값");
						
						
						$.each(jsonObj.data, function(i, jsonTemp){
							makerSel.options[i+1] = new Option(jsonTemp.maker, jsonTemp.maker);//new Option("보여질 값", "속성 값")
						});//each
					}//success
