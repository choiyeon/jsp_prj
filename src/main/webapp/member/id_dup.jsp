...
window.onload=function(){
	...
	document.getElementById("id").addEventListener("keydown", isEnter);//버튼 클릭
}
...
<div id="idWrap">
	...
	<!-- web  browser에서 키입력이 가능한 HTML Form Control이 하나인 경우
	  엔터를 치면 자동으로 submit이 된다.  ( 자바스크립트 에서 유효성검증을 실패하더라도 submit이 된다.)
	  이것을 막기위해서  키입력이 가능한 HTML Form Control을 하나더 정의하고 숨기면 웹브라우저에서는
	  키입력이 가능한 폼컨트롤이 두개라고 판단하여 엔터키가 눌려도 submit을 하지 않는다.
	 -->
