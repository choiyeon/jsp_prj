...
<div>
<%
String[] lunch={"오징어볶음","뼈다귀해장국","백반","돈가스","제육볶음"};
...
<%-- 	<c:out value="${ i.first } ${ i.last }"/> --%>
	...
	<c:if test="${ i.last }">
		<br>
		<c:out value="${ i.count }"/>건의 메뉴가 제안되었음.<br>
	</c:if>
</c:forEach>
...
<input type="text" name="name" value="${ name }" style="width:150px" ${ name eq "박시현" ? "readonly='readonly'":"" }/><br>
</c:forEach>
<hr/>
<%
